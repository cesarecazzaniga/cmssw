...
namespace edm {
  ...
  namespace {
    ...
    struct DirectionParameters {
      explicit DirectionParameters(const ParameterSet& pset)
        ...
        if (thetaMax <= thetaMin) {
          throw cms::Exception("DisplacedParticleGunProducerFlatEta") << "Please fix Momentum.Direction.ThetaMin/ThetaMax.";
        }
        if (phiMax <= phiMin) {
    ...
    /**
     * Samples theta with a FLAT-IN-ETA measure, over the set of
     * geometrically-allowed tan(theta) sub-intervals (produced by
     * getAllowedSlopesForCap/intersectIntervals upstream, entirely
     * unchanged from DisplacedParticleGunProducer -- only the probability
     * measure used to pick a value WITHIN those allowed sub-intervals
     * differs between the two producers).
     *
     * theta is monotonically DECREASING as a function of eta over the
     * forward range (0, pi/2) this producer operates in (eta = -ln(tan
     * (theta/2))), so each slope sub-interval's [thetaMin, thetaMax] maps
     * to an eta sub-interval with its endpoints FLIPPED: eta(thetaMax) is
     * the smaller eta bound, eta(thetaMin) is the larger one.
     *
     * This is this producer's WHOLE reason for existing (as opposed to
     * plain DisplacedParticleGunProducer, whose sampleTheta() samples flat
     * in theta -- NOT flat in eta: verified empirically that for a [1.5,
     * 3.0] eta window, flat-theta sampling gives ~4x more events near
     * eta=1.5 than near eta=3.0, since equal theta width subtends more
     * solid angle at larger theta/smaller eta), so unlike that producer,
     * there is no runtime choice here -- sampleDirection() below always
     * calls this function.
     */
    double sampleEta(CLHEP::HepRandomEngine* engine, const std::vector<Interval>& slopes) {
      ...
      for (const auto& slope : slopes) {
        ...
        // theta increasing -> eta decreasing, so the eta interval's
        // min/max are the FLIP of the theta interval's min/max.
      ...
      double sampledEta = etaIntervals.back().max;  // fallback, mirrors sampleTheta()'s own fallback pattern
      for (const auto& interval : etaIntervals) {
      ...
      // Convert back to theta -- everything downstream of this call
      // (projectToZ, momentum computation) is expressed in theta and is
      // completely unaware of which sampling measure produced it.
  ...
  class DisplacedParticleGunProducerFlatEta : public edm::global::EDProducer<> {
  public:
    explicit DisplacedParticleGunProducerFlatEta(const ParameterSet&);
    ~DisplacedParticleGunProducerFlatEta() override = default;
  ...
  DisplacedParticleGunProducerFlatEta::DisplacedParticleGunProducerFlatEta(const ParameterSet& pset)
  ...
  void DisplacedParticleGunProducerFlatEta::fillDescriptions(ConfigurationDescriptions& descriptions) {
    ...
    descriptions.add("DisplacedParticleGunProducerFlatEta", desc);
  }

  void DisplacedParticleGunProducerFlatEta::produce(edm::StreamID, edm::Event& event, const edm::EventSetup& setup) const {
    if (fParameters.verbosity > 0) {
      LogDebug("DisplacedParticleGunProducerFlatEta")
          << " DisplacedParticleGunProducerFlatEta : Begin New Event Generation" << std::endl;
    }
    ...
    if (fParameters.verbosity > 0) {
      LogDebug("DisplacedParticleGunProducerFlatEta")
          << " DisplacedParticleGunProducerFlatEta : Event Generation Done. " << std::endl;
    }
  }
...
using edm::DisplacedParticleGunProducerFlatEta;
DEFINE_FWK_MODULE(DisplacedParticleGunProducerFlatEta);